...
if(!qs_acscheck($user['level'], 'articles') && !qs_acscheck($user['level'], 'authorizer')){
	qs_redirect(107);
}
...
$sql2 = mysql_query("SELECT * FROM ".$qs_db['articles']." WHERE article_page='1' AND article_id='".$id."'");
...
if($count>1){
	for ($i=1; $i<=$count; $i++){
		$a = ($i == $p) ? 'a' : '' ;
		if ($i == 1){
			$pages = '<a '.$a.'href="admin.php?s=viewa&id='.$id.'&p='.$i.'">'.$i.'</a>';
		}
		else {
			$pages .= ' | <a '.$a.'href="admin.php?s=viewa&id='.$id.'&p='.$i.'">'.$i.'</a>';
		}
...
$adres = 'admin.php?s=viewa&id='.$id.'&p='.$p;
$cat = '2';
$id = $_GET['id'];
